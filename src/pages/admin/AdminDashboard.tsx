  ...
  useEffect(() => {
    (async () => {
      const [u, s, act, ina, pl, tg] = await Promise.all([
        ...
        supabase.from("songs").select("*", { count: "exact", head: true }).is("deleted_at", null),
        supabase.from("songs").select("*", { count: "exact", head: true }).is("deleted_at", null).eq("status", "active"),
        supabase.from("songs").select("*", { count: "exact", head: true }).is("deleted_at", null).eq("status", "inactive"),
